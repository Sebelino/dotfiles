...
--    , ((modm              , xK_q     ), spawn "xmonad --recompile; xmonad --restart")
    ...
    , ((0                 , 0x1008FF03), spawn "sudo asus-screen-brightness $(($(cat /sys/class/backlight/intel_backlight/brightness)-300))")
    -- Fn+F6: Brightness up
    , ((0                 , 0x1008FF02), spawn "sudo asus-screen-brightness $(($(cat /sys/class/backlight/intel_backlight/brightness)+300))")
    ...
    [((m .|. modm, key), screenWorkspace sc >>= flip whenJust (windows . f))
        | (key, sc) <- zip [xK_a, xK_q] [0..]  --SEBELINO
        , (f, m) <- [(W.view, 0), (W.shift, shiftMask)]]
